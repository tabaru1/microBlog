...
//		model.addAttribute("ftweet", ftweet);
	...
	*/
	...
	public String view(@PathVariable Integer id,Tweet tweet,Model model,@AuthenticationPrincipal UserDetails userDetails,RedirectAttributes ra) throws DataNotFoundException {
		String username = userDetails.getUsername();
		User userData = userService.findByEmail(username);
		User userId = userService.findById(userData.getId());
		model.addAttribute("user", userId);
		...
		try {
			...
			List<Tweet> tweets = tweetService.findByUserId(user.getId());
			model.addAttribute("tweets", tweets);
		} catch (DataNotFoundException e) {
			FlashData flash = new FlashData().danger("該当データがありません");
			ra.addFlashAttribute("flash", flash);
			return "redirect:/microBlog/admin/talk";		
			}
